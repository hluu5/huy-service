...
class IconTab extends React.Component {
    ...
    getNumOfCollections(video_id) {
        axios.get(`http://sidebar-component-env.phjkgcp7vm.us-east-2.elasticbeanstalk.com/collections`).then((data)=>{
            this.setState({
                numOfCollection: JSON.parse(data.data)
            })
        })
    }
    ...
    componentDidMount() {
        ...
        id = id.split('/');
        this.getNumOfComments(id[1] !== '/' ? Number(id[1]) : 1);
        this.getNumOfLikes(id[1] !== '/' ? Number(id[1]) : 1);
        this.getNumOfCollections(id[1] !== '/' ? Number(id[1]) : 1);
    }
